...
namespace DotNetDemo.Repository.Services
{
    public class UserAuthentication : IUserAuthentication
    {
        ...
        public async Task<Status> ForgotPasswordAsync(ForgotPasswordModel model)
        {
            var status = new Status();
            try
            {
                //Get User bu Email id
        ...
        public async Task<Status> LoginAsync(LoginModel model)
        {
            ...
            if (!await userManager.CheckPasswordAsync(user, model.Password))    //Check password of the user
            {
                status.StatusCode = 0;
